...
namespace internal_d2q9codegenubb_boundary_D2Q9CodegenUBB {
static FUNC_PREFIX void d2q9codegenubb_boundary_D2Q9CodegenUBB(uint8_t * RESTRICT const _data_indexVector, double * RESTRICT  _data_pdfs, int64_t const _stride_pdfs_0, int64_t const _stride_pdfs_1, int64_t const _stride_pdfs_2, int32_t indexVectorSize, double u_w, double v_w)
{
   ...
   for (int64_t ctr_0 = 0; ctr_0 < indexVectorSize; ctr_0 += 1)
   {
      ...
      _data_pdfs[_stride_pdfs_0*x + _stride_pdfs_0*f_in_inv_offsets_x[dir] + _stride_pdfs_1*y + _stride_pdfs_1*f_in_inv_offsets_y[dir] + _stride_pdfs_2*f_in_inv_dir_idx[dir]] = -rho*(u_w*6.0*((double)(neighbour_offset_x[dir])) + v_w*6.0*((double)(neighbour_offset_y[dir])))*weights[dir] + _data_pdfs[_stride_pdfs_0*x + _stride_pdfs_1*y + _stride_pdfs_2*dir];
   }
}
}
...
void D2Q9CodegenUBB::run_impl(IBlock * block, IndexVectors::Type type)
{
   ...
   internal_d2q9codegenubb_boundary_D2Q9CodegenUBB::d2q9codegenubb_boundary_D2Q9CodegenUBB(_data_indexVector, _data_pdfs, _stride_pdfs_0, _stride_pdfs_1, _stride_pdfs_2, indexVectorSize, u_w, v_w);
}

void D2Q9CodegenUBB::run(IBlock * block)
{
   run_impl(block, IndexVectors::ALL);
}

void D2Q9CodegenUBB::inner(IBlock * block)
{
   run_impl(block, IndexVectors::INNER);
}

void D2Q9CodegenUBB::outer(IBlock * block)
{
