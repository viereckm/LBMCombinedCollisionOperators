...
with CodeGeneration() as ctx:
    ...
    # LBM optimisation: use common subexpression elimination (CSE) when possible to save computational ressources (?)
    ...
    # Code generation:
    # LBM Sweep
    generate_sweep( ctx, 'D2Q9CumulantSweep', lbm_update_rule, field_swaps=[( pdfs, pdfs_tmp )], target=target )

    # Pack info
    generate_pack_info_from_kernel( ctx, 'D2Q9CumulantPackInfo', lbm_update_rule, target=target )

    # Macroscopic values setter (PDF field initializer)
    generate_sweep( ctx, 'D2Q9CumulantInitialPdfsSetter', pdfs_setter, target=target )

   # Boundaries:
    generate_boundary( ctx, "D2Q9CumulantNoSlip", NoSlip(), lbm_method, target=target )

    generate_boundary( ctx, "D2Q9CumulantUBB", UBB( velocity=wall_velocity, dim=stencil.D, data_type=data_type ), lbm_method, target=target )

    generate_boundary( ctx, "D2Q9CumulantSEO_E", SimpleExtrapolationOutflow( normal_direction=stencil[4], stencil=stencil ), lbm_method, target=target )

    # non-constant inflow boundary:

    generate_boundary( ctx, "D2Q9CumulantInflowFunc", inflowFuncUBB, lbm_method, target=target, additional_data_handler=inflowFuncUBBDataHandler )
