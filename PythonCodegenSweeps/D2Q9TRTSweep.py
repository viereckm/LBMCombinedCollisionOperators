...
with CodeGeneration() as ctx:
   ...
   # non-constant inflow UBB:
    ...
    # Code generation:
    # LBM Sweep
    generate_sweep( ctx, 'D2Q9TRTSweep', lbm_update_rule, field_swaps=[( pdfs, pdfs_tmp )], target=target )

    # Pack info
    generate_pack_info_from_kernel( ctx, 'D2Q9TRTPackInfo', lbm_update_rule, target=target )

    # Macroscopic values setter (PDF field initializer)
    generate_sweep( ctx, 'D2Q9TRTInitialPdfsSetter', pdfs_setter, target=target )

   # Boundaries:
    generate_boundary( ctx, "D2Q9TRTNoSlip", NoSlip(), lbm_method, target=target )

    generate_boundary( ctx, "D2Q9TRTUBB", UBB( velocity=wall_velocity, dim=stencil.D, data_type=data_type ), lbm_method, target=target )

    generate_boundary( ctx, "D2Q9TRTSEO_E", SimpleExtrapolationOutflow( normal_direction=stencil[4], stencil=stencil ), lbm_method, target=target )

    # non-constant inflow boundary:

    generate_boundary( ctx, "D2Q9TRTInflowFunc", inflowFuncUBB, lbm_method, target=target, additional_data_handler=inflowFuncUBBDataHandler )
