...
/**
 * @brief cellwise general (i.e. w/o specializations) SRT collision operator
 * 
 * @tparam LatticeModel_T lattice model defined by stencil (D2Q9 etc) and collision model
 * @tparam PdfField_T Pdf field corresponding to lattice model
 * @param lm lattice model used by simulation
 * @param src field to be modified (collision happens in place, thus nly one field needed)
 * @param x x coordinate of cell
 * @param y y coordinate of cell
 * @param z z coordinate of cell
 * @param rho local density
 * @param velocity local macroscopic velocity
 */
template< typename LatticeModel_T, typename PdfField_T, typename DensityVelocityOut_T >
void SRT_KERNEL( const LatticeModel_T & lm, PdfField_T * src, cell_idx_t x, cell_idx_t y, cell_idx_t z, real_t rho, Vector3< real_t > velocity, DensityVelocityOut_T densityVelocityOut )
{
    typedef typename LatticeModel_T::Stencil Stencil_T;

    const real_t omega = lm.collisionModel().omega( x, y, z, velocity, rho );

    densityVelocityOut( x, y, z, lm, velocity, rho );
    ...
    for( auto d = Stencil_T::begin(); d != Stencil_T::end(); ++d )
    {
        const real_t forceTerm = lm.forceModel().template forceTerm< LatticeModel_T >(
            ...
            rho, 
            commonForceTerms,
            LatticeModel_T::w[ d.toIdx() ],
            real_c( d.cx() ), real_c( d.cy() ), real_c( d.cz() ),
            omega, omega, omega
        );
...
/**
 * @brief cellwise general (i.e. w/o specializations) TRT collision operator
 * 
 * @tparam LatticeModel_T lattice model defined by stencil (D2Q9 etc) and collision model
 * @tparam PdfField_T Pdf field corresponding to lattice model
 * @param lm lattice model used by simulation
 ...
 */
template< typename LatticeModel_T, typename PdfField_T, typename DensityVelocityOut_T >
void TRT_KERNEL( const LatticeModel_T & lm, PdfField_T * src, cell_idx_t x, cell_idx_t y, cell_idx_t z, real_t rho, Vector3< real_t > velocity, DensityVelocityOut_T densityVelocityOut )
{
    typedef typename LatticeModel_T::Stencil Stencil_T;
...
template < typename LatticeModel_T, typename PdfField_T, typename DensityVelocityOut_T >
void D3Q27_CUMULANT_KERNEL( const LatticeModel_T & lm, PdfField_T * src, cell_idx_t x, cell_idx_t y, cell_idx_t z, real_t /* rho */, Vector3< real_t > /* velocity */, DensityVelocityOut_T densityVelocityOut )
{
    typedef typename LatticeModel_T::Stencil Stencil_T;
    ...
    densityVelocityOut( x, y, z, lm, Vector3< real_t >( velX, velY, velZ ), updated_rho );
